...
// ---------------------------------------------------------------------
// Analysis (historical 2020-2025 vs current season)
// ---------------------------------------------------------------------
function bindAnalysis() {
  ...
  document.getElementById("blockYieldMetric").addEventListener("change", () => {
    if (_analysisData) renderBlockYield(_analysisData);
  });
  document.getElementById("yieldPerTreeMetric").addEventListener("change", () => {
    if (_analysisData) renderYieldPerTreeHeatmap(_analysisData);
  });
  document.getElementById("varietyFilter").addEventListener("change", () => {
    if (_analysisData) renderVarietyYield(_analysisData);
  });

  // Delegated so it keeps working after any chart's own re-render, and
  // covers all seven "download as PDF" buttons with one listener.
...
async function loadAnalysis() {
  let data;
  try {
    data = await LW.api("/api/analysis/summary", { auth: true });
  } catch (e) {
    if (LW.isNetworkError(e)) { LW.setOffline(true); return; }
    if (LW.isAuthError(e)) { sessionExpired(); return; }
