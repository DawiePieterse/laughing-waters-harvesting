...
replaces the whole table each time rather than appending. (The app itself
also keeps the table current on every Weather tab open - see
weather.sync_recent_weather() - this script is for a full rebuild.)
...
def main():
    ...
    with Session(engine) as session:
        session.exec(delete(WeatherHistory))
